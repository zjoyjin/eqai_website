...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }) {
  const t = await getTranslations({ locale, namespace: 'page.kids' });
...
export default function KidsPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const breadcrumbJsonLd = generateJsonLd('BreadcrumbList', {
    itemListElement: [
      ...
      },
      {
        '@type': 'ListItem',
        position: 2,
        name: t('page.kids.title'),
        item: `${process.env.NEXT_PUBLIC_SITE_URL}/${locale}/kids`,
      },
    ],
  });
    ...
    <>
      ...
      <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 text-center">
          <span className="mb-4 inline-block text-6xl" role="img" aria-label={t('page.kids.title')}>
