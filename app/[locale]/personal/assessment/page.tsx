...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }) {
  ...
  return {
    title: `${t('category.personal.title')} - ${t('assessment.title')}`,
    description: t('assessment.description'),
  };
}

export default function PersonalAssessmentPage({ params: { locale } }: { params: { locale: string } }) {
      ...
      <div className="mx-auto max-w-screen-xl px-6 py-16 md:px-8 md:py-24">
        ...
        <div className="text-center">
          <h1 className="text-3xl font-semibold tracking-tight md:text-4xl">
            {t('category.personal.title')} - {t('assessment.title')}
          </h1>
