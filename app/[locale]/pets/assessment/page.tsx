...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }) {
  ...
  return {
    title: `${t('category.pets.title')} ${locale === 'zh' ? '评估' : 'Assessment'}`,
    description: locale === 'zh' ? '即将推出' : 'Coming soon',
  };
}

export default function PetsAssessmentPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      >
        ...
        {locale === 'zh' ? '返回宠物' : 'Back to Pets'}
      </Link>

      <div className="text-center">
        <span className="mb-6 inline-block text-7xl" role="img" aria-label="Pets Assessment">
          {t('category.pets.emoji')}
        </span>
        <h1 className="mb-4 text-4xl font-normal text-neutral-900">
          {t('category.pets.title')} {locale === 'zh' ? '评估' : 'Assessment'}
        </h1>
