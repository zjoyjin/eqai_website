...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }) {
  const t = await getTranslations({ locale, namespace: 'page.pet' });
...
export default function PetPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const breadcrumbJsonLd = generateJsonLd('BreadcrumbList', {
    itemListElement: [
      ...
      },
      {
        '@type': 'ListItem',
        position: 2,
        name: t('category.pet.title'),
        item: `${process.env.NEXT_PUBLIC_SITE_URL}/${locale}/pet`,
      },
    ],
  });
      ...
      <main id="main-content" className="flex-1">
        <div className="mx-auto max-w-screen-xl px-6 py-16 md:px-8 md:py-24">
          ...
          <p className="mt-2 max-w-2xl text-neutral-600">
            {t('category.pet.description')}
          </p>

          <div className="mt-8 rounded-xl border border-neutral-200 p-6">
            <p className="text-neutral-600 mb-4">{t('page.pet.content')}</p>
            <Link
              href={`/${locale}/pet/assessment`}
